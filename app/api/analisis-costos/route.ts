...
export async function GET(request: NextRequest) {
  try {
    ...
    const cacheKey = ocId ? CacheKeys.analisisCostos.byOC(ocId) : CacheKeys.analisisCostos.all()

    // Wrap entire cost analysis calculation in cache
    const result = await QueryCache.stats(
      cacheKey,
      async () => {
        ...
        const analisis = inventarios
          .map(inv => {
            ...
            const costoFinalCalculado =
              costoFobRD + pagosUnitario + gastosUnitario + comisionesUnitario
