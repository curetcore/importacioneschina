...
export async function GET() {
  try {
    ...
    // Pagos por método
    const pagosPorMetodo = todosPagos.reduce((acc, pago) => {
    ...
    // Pagos por tipo
    const pagosPorTipo = todosPagos.reduce((acc, pago) => {
    ...
    // Pagos por moneda
    const pagosPorMoneda = todosPagos.reduce((acc, pago) => {
    ...
    const tasasPorMoneda = todosPagos.reduce((acc, pago) => {
      if (pago.moneda !== "RD$") {
        const existente = acc.find((item) => item.moneda === pago.moneda);
    ...
    // Gastos por tipo
    const gastosPorTipo = todosGastos.reduce((acc, gasto) => {
    ...
    const inventarioPorBodega = ocsCalculadas.reduce((acc, oc) => {
    ...
    // Top productos (SKUs)
    const productosPorSKU = ocsCalculadas.reduce((acc, oc) => {
    ...
    const topProductos = [...productosPorSKU]
      .sort((a, b) => b.unidades - a.unidades)
      .slice(0, 5);

    // Compras por categoría
    const comprasPorCategoria = ocsCalculadas.reduce((acc, oc) => {
    ...
    const pagosRecientes = todosPagos.slice(0, 5).sort((a, b) => b.fechaPago.getTime() - a.fechaPago.getTime());
    const gastosRecientes = todosGastos.slice(0, 5).sort((a, b) => b.fechaGasto.getTime() - a.fechaGasto.getTime());
