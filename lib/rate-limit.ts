...
export async function rateLimit(config: RateLimitConfig): Promise<RateLimitResult> {
  const { limit = 10, windowSeconds = 10, identifier } = config
...
export const RateLimits = {
   ...
   */
  upload: (identifier: string) => rateLimit({ identifier, limit: 3, windowSeconds: 60 }),

  /**
   * Rate limit moderado para operaciones de escritura (POST/PUT/DELETE)
   * 20 requests cada 10 segundos
   */
  mutation: (identifier: string) => rateLimit({ identifier, limit: 20, windowSeconds: 10 }),

  /**
   * Rate limit permisivo para lecturas (GET)
   * 60 requests por minuto
   */
  query: (identifier: string) => rateLimit({ identifier, limit: 60, windowSeconds: 60 }),

  /**
   * Rate limit muy estricto para autenticación
   * 5 intentos cada 15 minutos
   */
  auth: (identifier: string) => rateLimit({ identifier, limit: 5, windowSeconds: 15 * 60 }),
}
